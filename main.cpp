...
int main()
{
    ...
    while (true)
    {
        ...
        if(detect_flag == 1){
            ...
            for(i = 0; i < befNum; i++){
                if(befTag[i].IP > 0){
                    rectangle(Process_Img, Point(befTag[i].rect.x, befTag[i].rect.y ), Point( befTag[i].rect.x+befTag[i].rect.w, befTag[i].rect.y+befTag[i].rect.h), Scalar( 0, 255, 255 ), 1, 8 );
                    sprintf(box_IP, "%d", befTag[i].IP - 1);
                    putText(Process_Img, box_IP, Point(befTag[i].rect.x, befTag[i].rect.y), FONT_HERSHEY_PLAIN, 1.0, Scalar(0, 255, 255), 1, 2, 0);
                }
            }
